...
# Cargar datos
with open(os.path.join(models_dir,"train_data.pkl"), "rb") as f:
    X_train_sparse, y_train = pickle.load(f)
with open(os.path.join(models_dir,"val_data.pkl"), "rb") as f:
...
with mlflow.start_run(run_name=f"Sparse - {model_name}"):
    ...
    try:
        ...
        with open(os.path.join(models_dir,"random_forest_val_f1.txt"), "w") as f:
            f.write(str(val_f1))
