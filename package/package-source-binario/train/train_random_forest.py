...
# Cargar datos
with open("clasificador/models/train_data.pkl", "rb") as f:
    X_train_sparse, y_train = pickle.load(f)
with open("clasificador/models/val_data.pkl", "rb") as f:
    X_val_sparse, y_val = pickle.load(f)
with open("clasificador/models/test_data.pkl", "rb") as f:
    X_test_sparse, y_test = pickle.load(f)
with open("clasificador/models/vectorizer.pkl", "rb") as f:
...
with mlflow.start_run(run_name=f"Sparse - {model_name}"):
    ...
    try:
        ...
        with open("clasificador/models/random_forest_model.pkl", "wb") as f:
            pickle.dump(best_model, f)
        with open("clasificador/models/random_forest_val_f1.txt", "w") as f:
            f.write(str(val_f1))
        
        print(f"  Val F1: {val_f1:.3f}, Test F1: {test_f1:.3f}")
        print(f"Modelo guardado como random_forest_model.pkl")
