...
try:
    train_dataset = load_from_disk("clasificador/models/train_dataset")
    val_dataset = load_from_disk("clasificador/models/val_dataset")
    test_dataset = load_from_disk("clasificador/models/test_dataset")
...
with mlflow.start_run(run_name=run_name):
    ...
    try:
        ...
        trainer.save_model(".clasificador/models/biobert_model")
        
        # Guardar F1 scores para el ensemble (IMPORTANTE para que funcione el ensemble)
        with open("clasificador/models/biobert_val_f1.txt", "w") as f:
            f.write(str(val_results['eval_f1']))
        with open("clasificador/models/biobert_test_f1.txt", "w") as f:
