...
test_dataset = torch.load("clasificador/models/test_dataset.pt")
...
for model_name in sparse_models:
    try:
        with open(f"clasificador/models/{model_name}_model.pkl", "rb") as f:
            model = pickle.load(f)
        with open(f"clasificador/models/{model_name}_val_f1.txt", "r") as f:
            f1_val = float(f.read())
...
try:
    with open("clasificador/models/biobert_val_f1.txt", "r") as f:
        contextual_val_f1 = float(f.read())
    with open("clasificador/models/biobert_test_f1.txt", "r") as f:
